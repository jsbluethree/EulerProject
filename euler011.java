...
public class euler011 {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < 20; ++i){
			for (int j = 0; j < 16; ++j){
				int test = nums[i][j] * nums[i][j+1] * nums[i][j+2] * nums[i][j+3]; 
				maxproduct = Math.max(maxproduct, test);
			}
		}
		// vertical
		for (int i = 0; i < 16; ++i){
			for (int j = 0; j < 20; ++j){
				int test = nums[i][j] * nums[i+1][j] * nums[i+2][j] * nums[i+3][j]; 
				maxproduct = Math.max(maxproduct, test);
			}
		}
		// diagonal right
		for (int i = 0; i < 16; ++i){
			for (int j = 0; j < 16; ++j){
				int test = nums[i][j] * nums[i+1][j+1] * nums[i+2][j+2] * nums[i+3][j+3]; 
				maxproduct = Math.max(maxproduct, test);
			}
		}
		//diagonal left
